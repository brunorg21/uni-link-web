...
interface AlocationTeacherFormProps {
  classroom: IRooms;
}

export function AlocationTeacherForm({ classroom }: AlocationTeacherFormProps) {
  const { user } = useUser();

  const { data: subjectsByTeacher } = useQuery({
    queryKey: ["subjects-by-teacher", user?.id],
    queryFn: async () =>
  ...
  const createAlocationMutation = useMutation<
    ...
    unknown
  >({
    mutationFn: async ({ classScheduleIds, classroomId, subjectId, date }) => {
